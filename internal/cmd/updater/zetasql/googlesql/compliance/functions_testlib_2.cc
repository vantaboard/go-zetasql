...
// Only struct equality/inequality is defined for GoogleSQL, not other
...
static std::vector<ComparisonTest> GetComparisonTests(
  ...
  if (include_array_comparisons) {
    ...
    for (const ComparisonTest& test : v) {
      ...
      GOOGLESQL_CHECK_OK(type_factory()->MakeArrayType(test.left.type(),
                                             &left_array_type));
      const Value left_array = values::Array(left_array_type, {test.left});

      const ArrayType* right_array_type;
      GOOGLESQL_CHECK_OK(type_factory()->MakeArrayType(test.right.type(),
                                             &right_array_type));
...
static std::vector<ArrayFirstLastTestCase> GetNullArrayFirstLastTestCases() {
  return {
...
static std::vector<ArrayFirstLastTestCase> GetEmptyArrayFirstLastTestCases() {
  return {
...
static std::vector<ArrayFirstLastTestCase> GetArrayFirstLastTestCases() {
  // a: string, b: int32
  const StructType* struct_type = SimpleStructType();
  const ArrayType* struct_array_type;
  GOOGLESQL_CHECK_OK(type_factory()->MakeArrayType(struct_type, &struct_array_type));
  const Value null_struct = Value::Null(struct_type);
  ...
  GOOGLESQL_CHECK_OK(type_factory()->MakeArrayType(nested_struct_type,
...
static std::vector<TypeFeaturePair> GetArrayTypesWithFeatures() {
...
static std::vector<QueryParamsWithResult> GetArraySliceTestCases(bool is_safe) {
...
static std::vector<QueryParamsWithResult> GetArrayFirstNLastNDesignDocTests(
  ...
  GOOGLESQL_CHECK_OK(type_factory()->MakeArrayType(i0.type(), &type));
...
static std::vector<TypeFeaturePair> GetOrderableTypesWithFeaturesAndValues() {
...
static std::vector<QueryParamsWithResult> GetArrayMinMaxTestCases(bool is_safe,
                                                                  bool is_min) {
  std::vector<QueryParamsWithResult> test_cases;
...
static std::vector<ArraySumAvgTestCase>
...
static std::vector<QueryParamsWithResult> GetArraySumTestCases(bool is_safe) {
...
static std::vector<QueryParamsWithResult> GetArrayAvgTestCases(bool is_safe) {
...
std::vector<QueryParamsWithResult> GetFunctionTestsAtOffset() {
  ...
  GOOGLESQL_CHECK_OK(type_factory()->MakeArrayType(struct_type, &array_struct_type));
  const Value struct0 = Value::Struct(struct_type, {String("foo"), Int32(0)});
  const Value struct1 = Value::Struct(struct_type, {String("bar"), Int32(1)});
